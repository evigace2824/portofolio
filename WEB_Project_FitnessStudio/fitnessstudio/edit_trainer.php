...
// Pull existing trainer
$stmt = $conn->prepare("SELECT * FROM trainers WHERE trainer_id = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($fullName === '' || $specialization === '') {
            throw new Exception("Name & specialization are required.");
        }
        $bio    = trim($_POST['bio']    ?? '');
        ...
        if (!empty($_FILES['image']['tmp_name'])
            ...
            if (!in_array($ext, ['jpg','jpeg','png','gif'])) {
                throw new Exception("Invalid image format. Use JPG/PNG/GIF.");
            }
        ...
        if (!$upd->execute()) {
            throw new Exception("DB error: " . $upd->error);
        }
        ...
        header("Location: edit_trainer.php?id={$id}");
        exit();
    }
}
?>
...
<head>
    ...
    <title>Edit Trainer – FitFlex</title>
...
<main>
    <div class="form-container">
        <div class="form-header">
            <h1><i class="fas fa-user-edit"></i> Edit Trainer</h1>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <?php if ($trainer['image_url']): ?>
                    <div class="current-image">
                        <img src="assets/images/<?= htmlspecialchars($trainer['image_url']) ?>" alt="Current trainer image">
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="full_name" name="full_name" required
                       value="<?= htmlspecialchars($trainer['full_name']) ?>">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.1" min="0" max="5" id="rating" name="rating"
                       value="<?= htmlspecialchars($trainer['rating']) ?>">
            </div>
