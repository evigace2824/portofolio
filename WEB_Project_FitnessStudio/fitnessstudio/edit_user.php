...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$first || !$last || !$usern || !$email) {
            throw new Exception('First, last, username, and email are required.');
        }
        $hash = null;
        if ($pw || $pw2) {
            if ($pw !== $pw2) throw new Exception('Passwords must match.');
        ...
        if ($hash) {
            $sql = "UPDATE users
                      SET first_name=?, last_name=?, date_of_birth=?, phone=?, address=?, role=?,
                          username=?, email=?, password_hash=?, updated_at=NOW()
                    WHERE user_id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param(
                'sssssssssi',
                $first, $last, $dob, $phone, $addr, $role,
                $usern, $email, $hash, $id
            );
        } else {
            $sql = "UPDATE users
